...
const loop = () => {
    ...
    // search results
    for (let i = 0; i < Class('res').length; i++){
        let res = Class('res')[i]
        let split = res.id.split('-')
        let sym = split[1]
        let n = JSON.parse(split[2])

        res.onclick = () => {
            let p = STATE.active_prob - 1
            if (sym == "i") {
                OBJECT.input_items[p] = OBJECT.input_results[p][n]
            } else {
                OBJECT.output_items[p] = OBJECT.output_results[p][n]
            }
            setActive()
            setTimeout(() => {
                drawHTML()
    ...
    for (let i = 0; i < Class("prob").length; i++) {
      ...
      div.onclick = () => {
        ...
        STATE.active_layers = [1, 1]
          ...
          console.log(OBJECT.input_results)
          setParams()
        setItems(ACTIVE.input_results, 'i')
        setItems(OBJECT.output_results[n-1], 'o')
        setTimeout(() => {
          drawHTML()
          drawItems()
        }, 100);
